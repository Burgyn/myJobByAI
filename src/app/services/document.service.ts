...
export class DocumentService {
  ...
  getPersonDocuments(tenantId: string, personId: string): Observable<FolderContent> {
    const url = `${this.baseUrl}/${tenantId}/documents/persons/${personId}`;
    return this.http.get<FolderContent>(url);
  }

  getPersonDocumentsForFolder(tenantId: string, personId: string, folderId: string): Observable<FolderContent> {
    const url = `${this.baseUrl}/${tenantId}/documents/persons/${personId}/${folderId}`;
    return this.http.get<FolderContent>(url);
  }

  downloadPersonDocument(tenantId: string, personId: string, documentId: string): Observable<Document> {
    const url = `${this.baseUrl}/${tenantId}/documents/persons/${personId}/document/${documentId}`;
    return this.http.get<Document>(url);
  }

  openPersonDocument(tenantId: string, personId: string, documentId: string): Observable<Document> {
